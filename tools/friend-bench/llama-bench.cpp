// friend.cpp: vendored from prismml/prism tools/llama-bench/llama-bench.cpp (the tree only kept main.cpp).
// Built by `make LLAMA_METAL=1 llama-bench`; not shipped.
#include <algorithm>
#include <array>
#include <cassert>
#include <chrono>
    ...
    llama_model_params to_llama_mparams() const {
        ...
        if (n_cpu_moe <= 0) {
            ...
            for (int i = 0; i < n_cpu_moe; ++i) {
                patterns.push_back(std::string("blk\\.") + std::to_string(i) + "\\.ffn_(up|down|gate)_(ch|)exps"); // friend.cpp: helper not in this tree
