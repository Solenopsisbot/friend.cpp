// friend.cpp: vendored from prismml/prism tests/test-backend-ops.cpp (this tree has no tests/ dir).
// Built by `make LLAMA_METAL=1 test-backend-ops`; not shipped. friend.cpp additions are marked.
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_eval() {
    ...
    // friend.cpp: every small-batch column count on the Bonsai types, with ragged row
    // counts (tails of the row tiling) and broadcast batches
    for (ggml_type type_a : {GGML_TYPE_Q1_0, GGML_TYPE_PQ2_0, GGML_TYPE_PTQ1_0}) {
        for (int64_t n = 1; n <= 17; n++) {
            test_cases.emplace_back(new test_mul_mat(type_a, GGML_TYPE_F32, 67,  n, 256,  {1, 1}, {1, 1}));
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
    ...
    // friend.cpp: small-batch (spec-decode verify / continuous batching) mat-vec on the
    // Bonsai 1-bit/ternary types at 27B shapes. m=34816 is the fused gate|up width (~25 MB
    // of Q1_0, bigger than the SLC so the timing is DRAM-bound like the real model).
    for (ggml_type type_a : {GGML_TYPE_Q1_0, GGML_TYPE_PQ2_0, GGML_TYPE_PTQ1_0, GGML_TYPE_Q4_0}) {
        for (int64_t n : {1, 2, 3, 4, 5, 6, 8, 12, 16, 512}) {
            test_cases.emplace_back(new test_mul_mat(type_a, GGML_TYPE_F32, 34816, n, 5120, {1, 1}, {1, 1}));
        }
    }
