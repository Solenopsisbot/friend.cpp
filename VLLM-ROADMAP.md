...
| [~] | 12. Per-request timing data | Batch results now expose queue, prefill, TTFT, decode and total timing fields, streaming metadata, and pause duration/count plus preemption count; live pause/resume coverage checks lifecycle fields. The data-parallel router adds epoch/worker and queue/upstream-header latency headers for cross-worker correlation. Cancellation and full end-to-end timing correlation remain. |
