...
void llama_model_dflash::load_arch_hparams(llama_model_loader & ml) {
    ...
    // AngelSpec DFly fuses the target context once per DRAFT layer. Detected from the fusion
    // tensor rather than a KV so a DFly export cannot load as plain DFlash.
    //
    // friend.cpp: upstream (#27310) folded the DFlash encoder into the decoder's KV-injection
    // batch, so the host now hands the draft the raw target features (n_embd_inp_enc wide) and
    // the per-layer fusion runs inside the decoder graph. Prism's original port widened
    // n_embd_inp/n_embd_out to n_layer*n_embd to round-trip the fused contexts through the
    // host; that is no longer needed (and would mis-size the token path), so it is not applied.
    if (ml.get_tensor_meta("layer_fusion")) {
        LLAMA_LOG_INFO("%s: DFly per-layer context fusion (n_layer = %u)\n", __func__, hparams.n_layer());
    }
...
// DFly (AngelSpec) per-draft-layer context fusion. The shared fc projection is only the base
// context: each draft layer adds its own softmax-weighted mix of the raw per-target-layer
// features, then the result goes through the post-fusion context_norm.
// Reference: Qwen3DFlyModel.project_target_hidden.
//
//   inp    : raw target features [n_feat*n_embd, n_tokens]
//   fc_out : fc(inp)             [n_embd, n_tokens]
//   returns                      [n_embd, n_layer, n_tokens] (layer-major within each token)
//
// friend.cpp: factored out of Prism's encoder graph so the fused KV-injection decoder path
// (upstream #27310) can run the same fusion in-graph.
static ggml_tensor * build_dfly_context_fusion(llm_graph_context & g, const llama_model & model,
        ggml_tensor * inp, ggml_tensor * fc_out) {
    ...
    GGML_ASSERT(n_feat*n_embd == (int64_t) g.hparams.n_embd_inp_enc());
    GGML_ASSERT(n_lyr == g.n_layer);
    ...
    ggml_tensor * cur = ggml_add(ctx0, resid, ggml_reshape_3d(ctx0, fc_out, n_embd, 1, n_tokens));
    cur = g.build_norm(cur, model.dfly_ctx_norm, NULL, LLM_NORM_RMS, -1);
    g.cb(cur, "dfly_ctx", -1);
...
// (kept for callers that still run llama_encode; the speculative driver uses the fused
// decoder path below)
template <>
llama_model_dflash::graph<true>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    if (model.dfly_layer_fusion) {
        // flatten layer-major within each token: one n_layer*n_embd row per token
        cur = build_dfly_context_fusion(*this, model, inp, cur);
        cur = ggml_reshape_2d(ctx0, cur, n_embd*n_layer, n_tokens);
...
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    // KV cache injection
    if (ubatch.embd) {
        // DFly fuses one context per draft layer (build_dfly_context_fusion); plain DFlash
        // injects a single shared context into every layer
        ...
        if (is_dfly) {
            inp_g = build_dfly_context_fusion(*this, model, inp_target, inp_g); // [n_embd, n_layer, n_tokens]
        } else {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // layer's own fused context ([n_embd, n_layer, n_tokens], layer-major per token)
            ggml_tensor * ctx_il = inp_g;
            if (is_dfly) {
                ctx_il = ggml_cont(ctx0, ggml_view_2d(ctx0, inp_g, n_embd, n_tokens,
                            inp_g->nb[2], (size_t) il*inp_g->nb[1]));
